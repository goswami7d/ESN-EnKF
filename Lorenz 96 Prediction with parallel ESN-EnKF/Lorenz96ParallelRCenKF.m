...
save('TrainedRCLorenz96Parallel.mat');

%%
% run the trained ESN in a generative mode. no need to initialize here, 
% because x is initialized with training data and we continue from there.
load('TrainedRCLorenz96Parallel.mat');
Y = zeros(stateSize,testLen);
Hdiag=zeros(1,stateSize);
i=1;
while i<=40
   Hdiag(i)=1;
   i=i+2;
end
%H=diag(double(rand(1,stateSize)>0.5));
%H=double(rand(stateSize,stateSize)>0.5);
H=diag(Hdiag);
EnsembleCovariance=0.01*eye(stateSize);
ObservationCovariance=0.001*eye(stateSize);
EnsembleSize=200;

%initState=data(trainLen+1,:);
initState=30*ones(1,40);
Ensemble=transpose(mvnrnd(initState,EnsembleCovariance,EnsembleSize));
xEnsemble=repmat(x,[1 1 EnsembleSize]);
...
for t = 1:testLen
 for j=1:EnsembleSize
  ...
  elseif i==stateSize
	...
	xEnsemble(:,i,j) = (1-a)*xEnsemble(:,i,j) + a*tanh( Win*u + W*xEnsemble(:,i,j) );
	y = Wout(:,:,i)*[1;u;xEnsemble(:,i,j)];
    %y = Wout(:,:,i)*[xEnsemble(:,i,j)];
	ytemp(i,j) = y;
	
  end
  ObservationForecast(:,j)=H*ytemp(:,j);
 end
  ObservationTrue=H*data(trainLen + t+1,:)'+transpose(mvnrnd(zeros(stateSize,1),ObservationCovariance));
  EnsembleMean=mean(ytemp,2);
  ObservationForecastMean=mean(ObservationForecast,2);
   for k=1:stateSize
       Ex(k,:)=[ytemp(k,:)-EnsembleMean(k)];
   end
   for k=1:stateSize
       Ey(k,:)=[ObservationForecast(k,:)-ObservationForecastMean(k)];
   end
   
   Pxy=Ex*Ey'/(EnsembleSize-1);
   Pyy=Ey*Ey'/(EnsembleSize-1)+ObservationCovariance;
   K=Pxy*inv(Pyy);
   y_est=ytemp+K*(ObservationTrue-ObservationForecast);
   y_estbar=mean(y_est,2);
...
save('Lorenz96RCEnKF.mat', 'correlation');
...
legend('Target signal', 'Estimated signal');
...
figure(6);
plot( vecnorm(data(trainLen+2:trainLen+errorLen+1,:)'-Y(:,1:errorLen)), 'b', 'linewidth',1);
hold off;
axis tight;
...
save('Lorenz96RCEnKFError.mat', 'MSETimeSeries');